!function(){
    ...
    module.exports = new Class({
        ...
        , _query: function(sql, callback) {
            ...
            if (debug) {
                log.info('['+this.id+'] Executing query: ' + sql);
                start = Date.now();
            }
            ...
            this.connection.query(sql, function(err, results) {
                ...
                if (debug){
                    time = Date.now()-start;
                    log.debug('['+this.id+'] Query took «'+time+'» msec ...');
                    log.debug('['+this.id+'] Query returned «'+(results && results.rows ? results.rows.length : 0 )+'» rows ...');
                }

                if ((results && results.rows ? results.rows.length : 0 ) > 1000) {
                    log.warn('['+this.id+'] Query returned «'+(results && results.rows ? results.rows.length : 0 )+'» records. Query:');
                    log(sql);
                }
