!function(){
    ...
    module.exports = new Class({
        ...
        , lock: function(schema, table, lockType, callback) {
            ...
            this._query('LOCK TABLE '+(schema? this._escapeId(schema)+'.': '')+this._escapeId(table)+' IN '+this._lockModes[lockType]+';', callback);
        }
        ...
        , _query: function(sql, callback) {
            ...
            this.connection.query(sql, function(err, results) {
                ...
                if (debug){
                    time = Date.now()-start;
                    log.debug('query took «'+time+'» msec ...');
                    log.debug('query returned «'+(results && results.rows ? results.rows.length : 0 )+'» rows ...');
                }

                if ((results && results.rows ? results.rows.length : 0 ) > 1000) {
                    log.warn('Query returned «'+(results && results.rows ? results.rows.length : 0 )+'» records. Query:');
                    log(sql);
                }
                    ...
                    if (type.object(results)) {
                        ...
                        switch (results.command) {
                            ...
                            case 'INSERT':
                                callback(null, {
                                      type: 'id'
                                    , id: results.rows && results.rows.length ? results.rows[0][Object.keys(results.rows[0])] : null
                                });
                                break;
