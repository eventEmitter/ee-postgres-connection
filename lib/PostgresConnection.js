            ...
            this.connection.query(sql, function(err, results){
                ...
                if (err) callback(err);
                    ...
                    if (type.object(results)) {
                        ...
                        if (type.array(results.rows)) { log.warn('returning rows');
                    ...
                    definitions.forEach(function(db){
                        ...
                        Object.keys(db.constraints).forEach(function(tableName){
                            ...
                            Object.keys(db.constraints[tableName]).forEach(function(constraintName){
                                ...
                                log.error(tableName);
                                //log(constraint);
                                //log(database[tableName]);

                                constraint.rules.forEach(function(rule){ log(rule.column_name);
                                    log.warn(constraint.type);
...
*/
        ...
        , listContraints: function(databaseName, callback){
            async.wait(function(done){
                ...
                //this.queryRaw('SELECT fkr.relname AS table_name, fka.attname AS column_name, pkr.relname AS referenced_table_name, pka.attname AS referenced_column_name , c.conname AS constraint_name FROM (((((((pg_constraint c JOIN pg_namespace cn ON ((cn.oid = c.connamespace))) JOIN pg_class fkr ON ((fkr.oid = c.conrelid))) JOIN pg_namespace fkn ON ((fkn.oid = fkr.relnamespace))) JOIN pg_attribute fka ON (((fka.attrelid = c.conrelid) AND (fka.attnum = ANY (c.conkey))))) JOIN pg_class pkr ON ((pkr.oid = c.confrelid))) JOIN pg_namespace pkn ON ((pkn.oid = pkr.relnamespace))) JOIN pg_attribute pka ON (((pka.attrelid = c.confrelid) AND (pka.attnum = ANY (c.confkey))))) WHERE (c.contype = \'f\'::"char") AND pkn.nspname = ?databaseName ORDER BY table_name, column_name;', {databaseName: databaseName}, done);
            ...
            function(done){ done();
                //this.queryRaw("SELECT tc.table_name , kcu.column_name , tc.constraint_type , tc.constraint_name FROM information_schema.table_constraints tc JOIN information_schema.key_column_usage kcu ON tc.constraint_name = kcu.constraint_name AND tc.table_name = kcu.table_name WHERE tc.constraint_schema = ?databaseName ANd kcu.constraint_schema = ?databaseName ORDER BY tc.table_name, tc.constraint_name, tc.constraint_type;", {databaseName: databaseName}, done);
                /*this.query({
